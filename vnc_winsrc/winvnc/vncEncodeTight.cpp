...
vncEncodeTight::SendFullColorRect(BYTE *dest, int w, int h)
{
	...
	要传送的内容以0x00开头？
	*/
	m_hdrBuffer[m_hdrBufferBytes++] = 0x00;//m_hdrBufferBytes是vncEncodeTight类里的一个成员变量。m_hdrBufferBytes记录的是m_hdrBuffer这个缓冲区里下一个空着的位置的地址。

	if (m_usePixelFormat24) {//m_usePixelFormat24是vncEncodeTight类里的一个成员变量
		Pack24(m_buffer, w * h);
		len = 3;//如果是24位每像素，那么每像素长度为3。
	} else
		len = m_remoteformat.bitsPerPixel / 8;//每像素长度
							 ...
							 int zlibLevel, int zlibStrategy)
{
	...
	*/
	if (dataLen < TIGHT_MIN_TO_COMPRESS) {
		memcpy(dest, m_buffer, dataLen);//destination source lenth
		return dataLen;
	}

	z_streamp pz = &m_zsStruct[streamId];//pz是个指针。
...
int
vncEncodeTight::SendJpegRect(BYTE *dst, int w, int h, int quality)//destination, width, height, quality
{
	/*
	用来管理jepg压缩的数据结构。
	*/
	...
	*/
	...
	cinfo.input_components = 3;//颜色分量（R、G、B）
	cinfo.in_color_space = JCS_RGB;

	jpeg_set_defaults(&cinfo);
	jpeg_set_quality(&cinfo, quality, TRUE);

	JpegSetDstManager (&cinfo, dst, w * h * (m_localformat.bitsPerPixel / 8));//设置压缩之后的数据目的地
	...
	m_hdrBuffer[m_hdrBufferBytes++] = rfbTightJpeg << 4;//以0x90结束

	return SendCompressedData(jpegDstDataLen);//压缩后的长度。
}
