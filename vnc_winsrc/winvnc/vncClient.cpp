...
void inline merge_rect(RECT& r1, RECT& r2, RECT& r) {
...
LONG inline area_of_rect(RECT& r) {
...
BOOL vncClient::SendUpdate()
{
	...
	try
	{
		...
		if (toBeSent.Rectangles(toBeSentList))
		{
			...
			const char* enc_name = m_buffer->GetEncoderName();
			bool is_rre = false;
			if (enc_name[0] == 'R' && enc_name[1] == 'R' && enc_name[2] == 'E' && enc_name[3] == 0) {
				is_rre = true;
				rectlist after_merge;
				for (auto& r1 : toBeSentList) {
					RECT merged;
					bool do_merged = false;
					for (auto& r2 : after_merge) {
						merge_rect(r1, r2, merged);
						if ((merged.right - merged.left) <= 64 || (merged.bottom - merged.top) <= 64 ||
							area_of_rect(merged) < (area_of_rect(r1) + area_of_rect(r2)) * 4) {
							r2 = merged;
							do_merged = true;
							break;
						}
					}
					if (!do_merged) after_merge.push_back(r1);
				}
				toBeSentList.swap(after_merge);

				RECT magic_rect;
				magic_rect.left = 0x123456;
				std::list<RECT>** pp_list = (std::list<RECT> **)(&magic_rect.top);
				*pp_list = new std::list<RECT>();
				(*pp_list)->swap(toBeSentList);
				toBeSentList.clear();
				toBeSentList.push_back(magic_rect);
			}
...
BOOL vncClient::SendRectangle(RECT &rect)
{
	...
	if(rect.left != 0x123456) IntersectRect(&rect, &rect, &sharedRect);
	else {
		std::list<RECT>** pp_list = (std::list<RECT>**)(&rect.top);
		for (auto& r : **pp_list) {
			IntersectRect(&r, &r, &sharedRect);
		}
	}
